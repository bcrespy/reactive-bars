export default {
  ...
  vertex: `
    ...
    void main() {
      ...
      noise = -.1 * turbulence( .5 * vNormal + iTime );
      float b = 5.0 * pnoise( 5.0 * position + iTime, vec3( 10.0 ) );
      float displacement = - noise + b;

      vec3 newPosition = position + vNormal * (0.05 + displace) *displacement;
  ...
  fragment: `
    ...
    void main() {
      ...
      vec2 coord1 = fract(vPosition.yz + cos(iTime)) * texScale;
      vec2 coord2 = fract(vPosition.zx + cos(iTime)) * texScale;
      vec2 coord3 = fract(vPosition.xy + cos(iTime)) * texScale;
